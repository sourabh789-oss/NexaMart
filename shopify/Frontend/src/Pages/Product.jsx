...
function Product() {
  ...
  const {
    ...
    staleTime: 1000 * 60 * 5,//5 minutes cache means no need to fetch this data until more than 5 minutes (inms time)
  });
          ...
          data?.map((element, idx) => {
            return <div key={idx} className=' px-7 py-4 flex flex-col items-center gap-4 rounded-xl border border-gray-300 border-solid  font-mono  box-border '>
              <div className='bg-[#f6f6f6] rounded-md w-[15rem] ' >
                <img loading="lazy" className=' w-[15rem] h-[15rem] object-contain transition-all cursor-pointer hover:scale-125' src={element.image} alt={`${element.category}`} />
              </div>
